...
class GPT(nn.Module):
    def __init__(self, config, pad_vocab_size_to=64):
        ...
        # Conditionally use toroidal embeddings depending on hyperparameters
        if config.use_toroidal_embed:
            self.transformer["wte"] = ToroidalEmbedding(
                padded_vocab_size, config.n_embd,
                block_size=config.torus_block_size,
                gain=config.torus_gain
            )
        else:
            self.transformer["wte"] = nn.Embedding(padded_vocab_size, config.n_embd)
